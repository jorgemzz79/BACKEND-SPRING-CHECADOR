...
public interface HoraRepository extends JpaRepository<Hora, Integer> {
    ...
    @Query(value = "SELECT *, " +
            "(SELECT TIME(fechaHora) AS checada FROM checadas WHERE DATE(fechaHora)=:fecha AND TIME(fechaHora) BETWEEN " +
            "desde AND hasta AND checadas.empleado_id=x.id_empleado LIMIT 1) AS puntual, " +
            "(SELECT TIME(fechaHora) AS checada FROM checadas WHERE DATE(fechaHora)=:fecha AND TIME(fechaHora) BETWEEN " +
            "TIME_FORMAT(SEC_TO_TIME(TIME_TO_SEC(desde)-3600), '%H:%i:%s') AND TIME_FORMAT(SEC_TO_TIME(TIME_TO_SEC(hasta)+3600), '%H:%i:%s') " +
            "AND checadas.empleado_id=x.id_empleado LIMIT 1) AS fuera " +
            "FROM (" +
            "SELECT e.id_empleado, h.entrada_salida, TIME_FORMAT( " +
            "CASE :dia " +
            ...
            nativeQuery = true)
    List<HorarioDiaDto> findHorariosPorDia(@Param("dia") String dia, @Param("fecha") Date fecha);
}
